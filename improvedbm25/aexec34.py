...
mydata=mytopicdb["cs2019_score_34"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2019_score_34_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_eleM_8 = log((25389659 - 3088 + 0.5) / (3088 + 0.5), 10)
    ...
    idf_eleK_1 = log((5435471 - 614 + 0.5) / (614 + 0.5), 10)
    idf_eleK_2 = log((5435471 - 158 +0.5) / (158 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        wordfreq = x['wordfreq']
        brugada = [True for x in wordfreq.items() if 'brugada' in x]
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'brugada' in key1:
                brugada_score = brugada_score + wordfreq[key]
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'syndrome' in key1:
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'scn5a' in key1:
                scn5a_score = scn5a_score + wordfreq[key]



        bm25_brugada_score = (((k1+1)*brugada_score)/((k1*(b1+(1-b1)*(len_freq/85)))+brugada_score))
        bm25_syndrome_score = (((k1 + 1) * syndrome_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + syndrome_score))
        bm25_scn5a_score = (((k1 + 1) * scn5a_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + scn5a_score))

        bm25_ab_score =idf_brugada*bm25_brugada_score+idf_syndrome*bm25_syndrome_score+idf_scn5a*bm25_scn5a_score

        idf_para=[{str(brugada_score):idf_brugada},{str(syndrome_score):idf_syndrome},{str(scn5a_score):idf_scn5a}]

        # ---------------共现分析摘要-------------------#
        if len(brugada) != 0 and brugada[0] and len(syndrome) != 0 and syndrome[0]:
            for key in wordfreq:
                key = cop.sub('', key)
                if 'scn5a' in key:
                    gx = idf_scn5a
    ...
    # ---------------共现分析化学-------------------#
        if len(brugada) != 0 and brugada[0] and len(syndrome) != 0 and syndrome[0]:
            for ele in ChemicalNameList:
                if 'SCN5A' in ele['NameOfSubstance']:
                    gx = idf_scn5a
                    break
     ...
     # ---------------共现分析医学主题词-------------------#
        if len(brugada) != 0 and brugada[0] and len(syndrome) != 0 and syndrome[0]:
            for eleM in MeshHeadingNameList:
                if 'SCN5A' in eleM['MeshHeadingName']:
                    gx = idf_scn5a
                    break


        for ele in ChemicalNameList:
            if 'Brugada Syndrome' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_1
                break
        for ele in ChemicalNameList:
            if 'Electrocardiography' == ele['NameOfSubstance']:
                ...
                break
        for ele in ChemicalNameList:
            if 'SCN5A protein, human' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_3
                break
        for ele in ChemicalNameList:
            if 'NAV1.5 Voltage-Gated Sodium Channel' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_4
                break
        for ele in ChemicalNameList:
            if 'Brugada Syndrome' == ele['NameOfSubstance']:
        ...
        for eleM in MeshHeadingNameList:
            if 'Brugada Syndrome' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_1
                break
        for eleM in MeshHeadingNameList:
            if 'Electrocardiography' == eleM['MeshHeadingName']:
                ...
                break
        for eleM in MeshHeadingNameList:
            if 'SCN5A protein, human' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_3
                break
        for eleM in MeshHeadingNameList:
            if 'NAV1.5 Voltage-Gated Sodium Channel' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_4
                break
        ...
        for eleK in KeywordsList:
            if 'brugada syndrome' in str(eleK).lower():
                ss4 = ss4 + idf_eleK_1
                break
        for eleK in KeywordsList:
            if 'scn5a' in str(eleK).lower():
