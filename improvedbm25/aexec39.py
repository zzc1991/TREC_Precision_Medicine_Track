...
mydata=mytopicdb["cs2019_score_39"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2019_score_39_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_kcnh2 = log((29138919 - 559 + 0.5) / (559 + 0.5), 10)

    idf_ele_1 = log((13670358 - 0 + 0.5) / (0 + 0.5), 10)
    idf_ele_2 = log((13670358 - 0+ 0.5) / (0 + 0.5), 10)
    idf_ele_3 = log((13670358 - 1708 + 0.5) / (1708 + 0.5), 10)
    idf_ele_4 = log((13670358 - 2776 + 0.5) / (2776 + 0.5), 10)
    ...
    idf_eleM_3 = log((25389659 - 1708 + 0.5) / (1708 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 2776+ 0.5) / (2776 + 0.5), 10)
    ...
    idf_eleK_2 = log((5435471 - 47 + 0.5) / (47 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'kcnh2' in key1:
                kcnh2_score = kcnh2_score + wordfreq[key]
        ...
        bm25_kcnh2_score = (((k1 + 1) * kcnh2_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + kcnh2_score))
        bm25_lqts_score = (((k1 + 1) * lqts_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + lqts_score))

        bm25_ab_score =idf_long*bm25_long_score+idf_syndrome*bm25_syndrome_score+idf_qt*bm25_qt_score+idf_kcnh2*bm25_kcnh2_score+idf_lqts*bm25_lqts_score

        idf_para=[{str(long_score):idf_long},{str(syndrome_score):idf_syndrome},{str(qt_score):idf_qt},{str(kcnh2_score):idf_kcnh2},{str(lqts_score):idf_lqts}]
    ...
    # ---------------共现分析化学-------------------#
        if len(long) != 0 and long[0] and len(qt) != 0 and qt[0] and len(syndrome) != 0 and syndrome[0]:
            for ele in ChemicalNameList:
                if 'KCNH2' in ele['NameOfSubstance']:
                    gx = idf_kcnh2
                    break
        if len(lqts) != 0 and lqts[0]:
            for ele in ChemicalNameList:
     ...
     # ---------------共现分析医学主题词-------------------#
        if len(long) != 0 and long[0] and len(qt) != 0 and qt[0] and len(syndrome) != 0 and syndrome[0]:
            for eleM in MeshHeadingNameList:
                if 'KCNH2' in eleM['MeshHeadingName']:
                    gx = idf_kcnh2
                    break
        if len(lqts) != 0 and lqts[0]:
            for eleM in MeshHeadingNameList:
        ...
        for ele in ChemicalNameList:
            if 'Electrocardiography' == ele['NameOfSubstance']:
                ...
                break
        for ele in ChemicalNameList:
            if 'ERG1 Potassium Channel' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_3
                break
        for ele in ChemicalNameList:
            if 'Ether-A-Go-Go Potassium Channels' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_4
                break
        ...
        for eleM in MeshHeadingNameList:
            if 'Electrocardiography' == eleM['MeshHeadingName']:
                ...
                break
        for eleM in MeshHeadingNameList:
            if 'ERG1 Potassium Channel' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_3
                break
        for eleM in MeshHeadingNameList:
            if 'Ether-A-Go-Go Potassium Channels' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_4
                break
