...
mydata=mytopicdb["cs2019_score_7"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2019_score_7_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_t790m = log((29138919 - 2 + 0.5) / (2 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 't790m' in key1:
                t790m_score = t790m_score + wordfreq[key]
        ...
        bm25_t790m_score = (((k1 + 1) * t790m_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 85))) + t790m_score))

        bm25_ab_score =idf_nonsmall*bm25_nonsmall_score+idf_cell*bm25_cell_score+idf_lung*bm25_lung_score+idf_nsclc*bm25_nsclc_score+idf_egfr*bm25_egfr_score+idf_t790m*bm25_t790m_score

        idf_para=[{str(nonsmall_score):idf_nonsmall},{str(cell_score):idf_cell},{str(lung_score):idf_lung},{str(nsclc_score):idf_nsclc},{str(egfr_score):idf_egfr},{str(t790m_score):idf_t790m}]
        ...
        if len(nonsmall) != 0 and nonsmall[0] and len(cell) != 0 and cell[0] and len(lung) != 0 and lung[0]:
            for ele in ChemicalNameList:
                if 'EGRF' in ele['NameOfSubstance']:
                    gx = idf_egfr
                if 'T790M' in ele['NameOfSubstance']:
                    gx1 = idf_t790m
                    break
        if len(nsclc) != 0 and nsclc[0]:
            for ele in ChemicalNameList:
     ...
     # ---------------共现分析医学主题词-------------------#
        if len(nonsmall) != 0 and nonsmall[0] and len(cell) != 0 and cell[0] and len(lung) != 0 and lung[0]:
            for eleM in MeshHeadingNameList:
                if 'EGRF' in eleM['MeshHeadingName']:
                    gx = idf_egfr
                if 'T790M' in eleM['MeshHeadingName']:
                    gx1 = idf_t790m
                    break
        if len(nsclc) != 0 and nsclc[0]:
            for eleM in MeshHeadingNameList:
        ...
        for ele in ChemicalNameList:
            if 'ErbB Receptors' == ele['NameOfSubstance']:
                ...
                break
        for ele in ChemicalNameList:
            if 'Protein Kinase Inhibitors' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_4
                break
        for ele in ChemicalNameList:
            if 'Quinazolines' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_5
                break
        for ele in ChemicalNameList:
        ...
        for eleM in MeshHeadingNameList:
            if 'ErbB Receptors' == eleM['MeshHeadingName']:
                ...
                break
        for eleM in MeshHeadingNameList:
            if 'Protein Kinase Inhibitors' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_4
                break
        ...
        for eleM in MeshHeadingNameList:
            if re.findall(r'(Adult|Adults)', eleM['MeshHeadingName']):
                ...
                break
        for eleM in MeshHeadingNameList:
            if 'Quinazolines' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_10
                break
        for eleM in MeshHeadingNameList:
