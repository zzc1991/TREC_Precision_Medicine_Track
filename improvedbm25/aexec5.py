...
mydata=mytopicdb["cs2019_score_5"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2019_score_5_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_s310y = log((29138919 - 6 + 0.5) / (6 + 0.5), 10)
    ...
    idf_eleM_1 = log((25389659 - 70343 + 0.5) / (70343 + 0.5), 10)
    idf_eleM_2 = log((25389659 - 33210 + 0.5) / (33210 + 0.5), 10)
    idf_eleM_3 = log((25389659 - 313452 + 0.5) / (313452 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 's310y' in key1:
                s310y_score = s310y_score + wordfreq[key]

        bm25_cervical_score = (((k1+1)*cervical_score)/((k1*(b1+(1-b1)*(len_freq/85)))+cervical_score))
        bm25_erbb2_score =(((k1+1)*erbb2_score)/((k1*(b1+(1-b1)*(len_freq/85)))+erbb2_score))
        bm25_s310y_score = (((k1+1)*s310y_score)/((k1*(b1+(1-b1)*(len_freq/85)))+s310y_score))
        bm25_ab_score =idf_cervical*bm25_cervical_score+idf_erbb2*bm25_erbb2_score+idf_s310y*bm25_s310y_score

        idf_para=[{str(cervical_score):idf_cervical},{str(erbb2_score):idf_erbb2},{str(s310y_score):idf_s310y}]
    ...
    # ---------------共现分析化学-------------------#
        if len(cervical) != 0 and cervical[0]:
            for ele in ChemicalNameList:
                if 'S310Y' in ele['NameOfSubstance']:
                    gx1 = idf_s310y
                    break
        if len(cervical) != 0 and cervical[0]:
            for ele in ChemicalNameList:
     ...
     # ---------------共现分析医学主题词-------------------#
        if len(cervical) != 0 and cervical[0]:
            for eleM in MeshHeadingNameList:
                if 'S310Y' in eleM['MeshHeadingName']:
                    gx1 = idf_s310y
                    break
        if len(cervical) != 0 and cervical[0]:
            for eleM in MeshHeadingNameList:
        ...
        for ele in ChemicalNameList:
            if 'Uterine Cervical Neoplasms' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_1
                break
        for ele in ChemicalNameList:
            if 'Cervical Vertebrae' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_2
                break
        for ele in ChemicalNameList:
            if 'Radiography' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_3
                break
        for ele in ChemicalNameList:
        ...
        for eleM in MeshHeadingNameList:
            if 'Uterine Cervical Neoplasms' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_1
                break
        for eleM in MeshHeadingNameList:
            if 'Cervical Vertebrae' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_2
                break
        for eleM in MeshHeadingNameList:
            if 'Radiography' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_3
                break
        for eleM in MeshHeadingNameList:
