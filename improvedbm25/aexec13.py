...
mydata=mytopicdb["cs2019_score_13"]#按词表长度改进过后的2次排序表
mycount = mytopicdb["cs2019_score_13_related"]#聚类后对应与主题相关联的文献
...
def sortsecond(myfreq,mydata,yuzhi):
    ...
    idf_ezrros1 = log((29138919 - 7 + 0.5) / (7 + 0.5), 10)
    idf_fusion = log((29138919 - 167225 + 0.5) / (167225 + 0.5), 10)

    idf_ele_1 = log((13670358 - 0 + 0.5) / (367 + 0.5), 10)
    idf_ele_2 = log((13670358 - 33948 + 0.5) / (33948 + 0.5), 10)
    idf_ele_3 = log((13670358 - 56642 + 0.5) / (56642 + 0.5), 10)
    ...
    idf_eleM_3 = log((25389659 - 33948 + 0.5) / (33948 + 0.5), 10)
    idf_eleM_4 = log((25389659 - 56642 + 0.5) / (56642 + 0.5), 10)
    ...
    idf_eleM_8 = log((25389659 - 46148 + 0.5) / (46148 + 0.5), 10)
    ...
    for x in myfreq.find({}, {'PMID', 'wordfreq', 'ChemicalNameList', 'MeshHeadingNameList', 'KeywordsList'},
        ...
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'ezr-ros1' in key1:
                ezrros1_score = ezrros1_score + wordfreq[key]
        for key in wordfreq:
            key1 = cop.sub('', key)
            if 'fusion' in key1:
        ...
        if len(lung) != 0 and lung[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0]:
            for ele in ChemicalNameList:
                if 'EZR-ROS1' in ele['NameOfSubstance']:
                    gx = idf_ezrros1
                    break
        ...
        if len(lung) != 0 and lung[0] and len(adenocarcinoma) != 0 and adenocarcinoma[0]:
            for eleM in MeshHeadingNameList:
                if 'EZR-ROS1' in eleM['MeshHeadingName']:
                    gx = idf_ezrros1
                    break
        ...
        bm25_ezrros1_score = (((k1 + 1) * ezrros1_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + ezrros1_score))
        bm25_fusion_score = (((k1 + 1) * fusion_score) / ((k1 * (b1 + (1 - b1) * (len_freq / 83))) + fusion_score))

        bm25_ab_score = idf_lung * bm25_lung_score + idf_adenocarcinoma * bm25_adenocarcinoma_score + idf_ezrros1 * bm25_ezrros1_score + idf_fusion * bm25_fusion_score

        idf_para = [{str(lung_score): idf_lung}, {str(adenocarcinoma_score): idf_adenocarcinoma},
                    {str(ezrros1_score): idf_ezrros1}, {str(fusion_score): idf_fusion}]
        ...
        for ele in ChemicalNameList:
            if 'Protein-Tyrosine Kinases' == ele['NameOfSubstance']:
                ...
                break
        for ele in ChemicalNameList:
            if 'Proto-Oncogene Proteins' == ele['NameOfSubstance']:
                ss1 = ss1 + idf_ele_4
                break
        for ele in ChemicalNameList:
        ...
        for eleM in MeshHeadingNameList:
            if 'Proto-Oncogene Proteins' == eleM['MeshHeadingName']:
                ss2 = ss2 + idf_eleM_4
                break
        for eleM in MeshHeadingNameList:
