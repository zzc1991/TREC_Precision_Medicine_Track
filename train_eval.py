...
def train(config, model, train_dataloader, dev_dataloader, test_dataloader):
    ...
    for epoch in range(0, config.num_epochs):
        ...
        for i, trains in enumerate(train_dataloader):
            ...
            model.zero_grad()
            loss, logit = outputs[0], outputs[1]  # outputs[0]: loss, outputs[1]: predict  (batch_size, num_classes)
            loss.backward()
...
def test(config, model, dataloader):
    ...
    test_acc, test_loss, test_report, test_confusion,result = evaluate(config, model, dataloader, test=True)
    t0 = time.time()
...
def evaluate(config, model, dataloader, test=False):
    ...
    with torch.no_grad():
        for i, dev in enumerate(dataloader):
            ...
            PMIDS = dev[4].to(config.device)
            outputs = model(input_ids=input_ids, attention_mask=mask_ids, token_type_ids=segment_ids, labels=labels)
            loss, logit = outputs[:2]

            labels = labels.data.cpu().numpy()
            predic = torch.max(logit.data, 1)[1].cpu().numpy()
            PMIDS = PMIDS.data.cpu().numpy()
            softmax_res = F.softmax(logit, dim=1)
            softmax_list=[]
            for item in softmax_res.tolist():
                softmax_list.append(item[1])
            for item in zip(PMIDS,labels,predic,softmax_list):
    ...
    if test:
        report = ''
        confusion = ''
        return acc, loss_total / len(dataloader), report, confusion,result
    return acc, loss_total / len(dataloader),result
